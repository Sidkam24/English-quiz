...
//文末なしの文頭小文字
 var eigo2 =["good morning","hello","nice to meet you","how are you","goodbye","i don't know","what's the date today","what day is it today","good night","how old are you"
         ,"where are you from","good evening","see you","thank you","i'm sleepy","what time is it now","what do you want to be"];
//文末ありの文頭小文字
...
//         ,"where are you from?","good evening.","see you.","thank you.","I'm sleepy.","what time is it now?","here you are."];
//文末ありの文頭大文字
var eigo4 =["Good morning.","Hello.","Nice to meet you.","How are you?","Goodbye.","I don't know.","What's the date today?","What day is it today?","Good night.","How old are you?"
        ,"Where are you from?","Good evening.","I like soccer.","Thank you.","I'm sleepy.","What time is it now?","What do you want to be?"];
...
　　　　　　　 "<ruby>今日<rp>(</rp><rt>きょう</rt><rp>)</rp></ruby>は<ruby>何曜日<rp>(</rp><rt>なんようび</rt><rp>)</rp></ruby>ですか？","おやすみ","<ruby>何歳<rp>(</rp><rt>なんさい</rt><rp>)</rp></ruby>ですか？",
              "<ruby>出身<rp>(</rp><rt>しゅっしん</rt><rp>)</rp></ruby>はどこですか？","こんばんは","<ruby>私<rp>(</rp><rt>わたし</rt><rp>)</rp></ruby>はサッカーが<ruby>好<rp>(</rp><rt>す</rt><rp>)</rp></ruby>きです",
              "ありがとう！","<ruby>眠<rp>(</rp><rt>ねむ</rt><rp>)</rp></ruby>たいです",
　　　　　　   "<ruby>今何時<rp>(</rp><rt>いまなんじ</rt><rp>)</rp></ruby>ですか？","<ruby>何<rp>(</rp><rt>なに</rt><rp>)</rp></ruby>になりたいですか？"];
...
function stoptimer(){
  ...
  function restart(){　//countup関数はローカル関数なため
      sec++;
      if(sec>99){
        sec=0;
        min=min+1;
...
function get(){
  ...
  var monkazu = $("#monkazu").text();
	var score = tno*monkazu;
	score = score*100;
...
//エンターキーで送信や次へを押す
$("#text").on('keypress',function(e){ //テキストの中にフォーカスしているとき
	if(tensu <= kei_max){　//点数が最大問題数以下のとき。これを設定しないと最大問題数が終わっても送信や次へをし続ける。
	　　　　　　　　　　	//（disabledになっていたとしても送信や次へを押せるから。
  if(e.which == 13){　//エンターキーを押したとき
      var dis = $("#sousin").is(":disabled");//送信ボタンがdisabaleかどうかをtrue,falseで判別
	  var dis2 = $("#next").is(":disabled");//次へボタンがdisableかどうかをtrue,falseで判別
    if(!dis){　//送信ボタンがdisableではないとき、送信を押せる。こうしないとdisableの時も押せちゃうから
    $("#sousin").click();
    }
      else if(dis==true && dis2==false){　//送信がdisableで次へがdisabaleでないとき。
		  　　　　　　　　　　　　　　　　//else if(dis)だと、最後終わったとき両方disable状態でも次へ押されちゃうから
      $("#next").click();
    }
  }
}
...
$("#onsei").on('keypress',function(e){　//音声ボタンが押された時
	if(tensu <= kei_max){
		var input = $("#text").val();
		if(input !== ""){　//テキストの中身が空であれば送信しない。
  if(e.which == 13){
  ...
  //送信ボタンを押した後
    ...
    onseistop2();
       ...
       if( input === eigo2[rand] ){ //テキストの中身と答えが一致するなら
        ...
        if(eigo2.length === 1){　//問題を削除しすぎてなくならないように、問題数を初期化する。
           eigo4 =["Good morning.","Hello.","Nice to meet you.","How are you?","Goodbye.","I don't know.","What's the date today?","What day is it today?","Good night.","How old are you?"
                  ,"Where are you from?","Good evening.","I like soccer.","Thank you.","I'm sleepy.","What time is it now?","What do you want to be?"];
           nihongo =["おはよう","こんにちは","はじめまして","お<ruby>元気<rp>(</rp><rt>げんき</rt><rp>)</rp></ruby>ですか？","さようなら","わかりません",
                        "<ruby>今日<rp>(</rp><rt>きょう</rt><rp>)</rp></ruby>の<ruby>日付<rp>(</rp><rt>ひづけ</rt><rp>)</rp></ruby>は<ruby>何<rp>(</rp><rt>なん</rt><rp>)</rp></ruby>ですか？",
          　　　　　　　 "<ruby>今日<rp>(</rp><rt>きょう</rt><rp>)</rp></ruby>は<ruby>何曜日<rp>(</rp><rt>なんようび</rt><rp>)</rp></ruby>ですか？","おやすみ","<ruby>何歳<rp>(</rp><rt>なんさい</rt><rp>)</rp></ruby>ですか？",
                        "<ruby>出身<rp>(</rp><rt>しゅっしん</rt><rp>)</rp></ruby>はどこですか？","こんばんは","<ruby>私<rp>(</rp><rt>わたし</rt><rp>)</rp></ruby>はサッカーが<ruby>好<rp>(</rp><rt>す</rt><rp>)</rp></ruby>きです",
                        "ありがとう！","<ruby>眠<rp>(</rp><rt>ねむ</rt><rp>)</rp></ruby>たいです",
          　　　　　　   "<ruby>今何時<rp>(</rp><rt>いまなんじ</rt><rp>)</rp></ruby>ですか？","<ruby>何<rp>(</rp><rt>なに</rt><rp>)</rp></ruby>になりたいですか？"];
           eigo2 =["good morning","hello","nice to meet you","how are you","goodbye","i don't know","what's the date today","what day is it today","good night","how old are you"
                  ,"where are you from","good evening","i like soccer","thank you","i'm sleepy","what time is it now","what do you want to be"];

                  // $.each(eigo4,function(index,val){
                  //   var ass = val.toLowerCase();
                  //   as = as.replace(/[\.\?!]$/,'');
                  //   eigo2.push(as);
                  // });
        }

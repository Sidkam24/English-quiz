...
function onsei(){  //Web Speech Recognition API
  ...
  $("#onsei").click(function(){
    ...
    //※遊び心　指定の言葉を音声で入力すると移動する
      ...
      else if(text === "Dictionary"){ //"Dictionary"と叫ぶと辞書に飛ぶ
...
//timerの設置
function  timer(){
  $("#text").on("keyup",function(){　//テキストボックスにフォーカスしボタンを押したら
    if(flag==0){
      start();
      flag=1;
    }
  });
  $("#onsei").on("click",function(){　//音声開始をクリックしたら
    if(flag==0){
      start();
      flag=1;
    }
  });
  $("#sousin").on("click",function(){　//送信ボタンを押したら
    if(flag==0){
      start();
      flag=1;
    }
...
//エンターキーで送信と次へ
$("#text").on('keypress',function(e){　//テキストの中にカーソルがあるとき
  if(kei2 <= kei_max){
  if(e.which == 13){
...
$("#onsei").on('keypress',function(e){　//音声ボタンを押しているとき
  if(kei2 <= kei_max){
  if(e.which == 13){
    var dis = $("#sousin").is(":disabled");
    if(!dis){
